...
type Props = {
  status: string;
  message: string;
  onValidated: (opts: OnValidatedInputType) => string;
};
const NewsletterForm = ({ status, message, onValidated }: Props) => {
  ...
  return (
    <div className="flex flex-col text-left items-left max-w-4xl mx-auto">
      <div className="grow p-3 text-2xl">
        Subscribe to our newsletter to receive special offers and first look at
        new products 😍
      </div>
      <div className="px-3">
        <input
